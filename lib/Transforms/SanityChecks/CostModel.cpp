...
//===----------------------------------------------------------------------===//
//
// Adapted from LLVM's Analysis/CostModel.cpp. We transformed the CostModel
// from a pass to a standalone function that can be called from anywhere.
//
...
unsigned getInstructionCost(const Instruction *I, const TargetTransformInfo *TTI) {
  if (!TTI)
    return -1;
