...
// RUN: clang -Wall -O1 -flto -fsanitize-coverage=edge,indirect-calls,8bit-counters -fsanitize=address -c -o %t.orig.o %s
// RUN: clang -Wall -O1 -flto -fsanitize-coverage=edge,indirect-calls,8bit-counters -fsanitize=address -fsanitize=asap -mllvm -asap-cost-threshold=0 -c -o %t.o %s
