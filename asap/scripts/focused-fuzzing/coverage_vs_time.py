...
def parse_logs(f):
    ...
    for line in f:
        ...
        if m:
            ...
            print("Current timestamp: {} delta: {}".format(current_timestamp, current_timestamp - start_timestamp))
            continue
...
def main():
    ...
    for current_time in range(end_time):
        ...
        last_execs = events_up_to_now[-1][2]
        print("{:8d}\t{:8d}\t{:12d}".format(current_time, last_coverage, last_execs))

    print("{:8d}\t{:8d}\t{:12d}".format(events[-1][0], last_coverage, events[-1][2]))
